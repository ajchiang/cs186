...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2 * d) {
            ...
            LeafNode right = new LeafNode(metadata, bufferManager, rkeys, rrids, rightSibling, treeContext);
            ...
            sync();
            return Optional.of(new Pair<>(rkeys.get(0), right.getPage().getPageNum()));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        if (keys.size() > limit) {
            ...
            keys1.add(keys.remove(keys.size() - 1));
            rids1.add(rids.remove(rids.size() - 1));
            LeafNode node = new LeafNode(metadata, bufferManager, keys1, rids1, this.rightSibling, treeContext);
            long pagenum = node.getPage().getPageNum();
            this.rightSibling = Optional.of(pagenum);

            sync();
            return Optional.of(new Pair<>(keys1.get(0), pagenum));
        }
        sync();
        return Optional.empty();
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long s = buf.getLong();
        Optional<Long> rightSibling = Optional.of(s);
