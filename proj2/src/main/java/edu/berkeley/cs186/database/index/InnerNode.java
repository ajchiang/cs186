...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        while (!(child instanceof LeafNode)) {
            child = child.get(key);
        }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int index = numLessThanEqual(key, keys);
        BPlusNode node = BPlusNode.fromBytes(metadata, bufferManager, treeContext, children.get(index));
        Optional<Pair<DataBox, Long>> temp = node.put(key, rid);

        if (!temp.isPresent()) {
            return Optional.empty();
        }

        int index1 = numLessThanEqual(temp.get().getFirst(), keys);
        keys.add(index1, temp.get().getFirst());
        children.add(index1 + 1, temp.get().getSecond());

        if (keys.size() > 2 * d) {
            List<DataBox> lkeys = keys.subList(0, d);
            DataBox middle = keys.get(d);
            List<DataBox> rkeys = keys.subList(d, 2 * d + 1);
            List<Long> lchild = children.subList(0, d + 1);
            List<Long> rchild = children.subList(d + 1, 2 * d + 2);

            InnerNode temp1 = new InnerNode(metadata, bufferManager, rkeys, rchild, treeContext);
            rkeys.remove(0);
            this.keys = lkeys;
            this.children = lchild;

            sync();
            return Optional.of(new Pair<>(middle, temp1.getPage().getPageNum()));
        } else {
            sync();
            return Optional.empty();
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() <= d * 2) {
            ...
            temp = rightmostChild.bulkLoad(data, fillFactor);
            if (temp.isPresent()) {
        ...
        if (keys.size() > d * 2) {
            ...
            child1.add(children.remove(d + 1));
            while (d + 1 < keys.size()) {
                keys1.add(keys.remove(d + 1));
                child1.add(children.remove(d + 1));
            }
            DataBox splitKey1 = keys.remove(d);
            InnerNode rightInnerNode = new InnerNode(metadata, bufferManager, keys1, child1, treeContext);
            long rightInnerNodePageNum = rightInnerNode.getPage().getPageNum();

            sync();
            return Optional.of(new Pair<>(splitKey1, rightInnerNodePageNum));
