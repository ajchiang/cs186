...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Pair<DataBox, Long> temp1 = temp.get();

        List<Long> children = new ArrayList<>();
        List<DataBox> keys = new ArrayList<>();

        keys.add(temp1.getFirst());
        children.add(root.getPage().getPageNum());
        children.add(temp1.getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        while (data.hasNext()) {
            Optional<Pair<DataBox, Long>> temp = this.root.bulkLoad(data, fillFactor);
            if (temp.isPresent()) {
                List<DataBox> keys1 = new ArrayList<>();
                List<Long> child1 = new ArrayList<>();
                keys1.add(temp.get().getFirst());
                child1.add(root.getPage().getPageNum());
                child1.add(temp.get().getSecond());
                this.root = new InnerNode(metadata, bufferManager, keys1, child1, lockContext);
            }
        }
    }
