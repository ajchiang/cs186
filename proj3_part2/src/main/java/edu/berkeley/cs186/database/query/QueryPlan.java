...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        newSet.add(startTableName);
        Map<Set, QueryOperator> map1 = new HashMap<>();
        map1.put(newSet, minCostSingleAccess(startTableName));

        for (String name : joinTableNames) {
            newSet = new HashSet<>();
            newSet.add(name);
            map1.put(newSet, minCostSingleAccess(name));
        }
        ...
        Map<Set, QueryOperator> previous = new HashMap<>(map1);
        while (previous.size() > 1) {
            previous = minCostJoins(previous, map1);
        }
        ...
        finalOperator = this.minCostOperator(previous);
        addGroupBy();
        addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for (int index : getEligibleIndexColumns(table)) {
            PredicateOperator pred = selectOperators.get(index);
            DataBox value = selectDataBoxes.get(index);
            op = new IndexScanOperator(transaction, table, selectColumnNames.get(index), pred, value);
            if (op.getIOCost() < minCost) {
                minOp = op;
                except = index;
            }
        }
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for (Map.Entry<Set, QueryOperator> pair : prevMap.entrySet()) {
            for (int i = 0; i < joinTableNames.size(); i++) {
                ...
                String [] leftCol = getJoinLeftColumnNameByIndex(i);
                String [] rightCol = getJoinRightColumnNameByIndex(i);

                if (pair.getKey().contains(leftCol[0]) && !pair.getKey().contains(rightCol[0])) {
                    newSet = new HashSet(Collections.singleton(rightCol[0]));
                    rightOp = pass1Map.get(newSet);
                    leftOp = pair.getValue();
                } else if (!pair.getKey().contains(leftCol[0]) && pair.getKey().contains(rightCol[0])) {
                    newSet = new HashSet(Collections.singleton(leftCol[0]));
                    leftOp = pass1Map.get(newSet);
                    rightOp = pair.getValue();
                } else {
                    continue;
                }

                minJoin = minCostJoinType(leftOp, rightOp, leftCol[1], rightCol[1]);
